...
class Ui_UpdateNote(object):
    ...
    def fillData(self):
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(f"SELECT skrot_sprawdzianu FROM sprawdziany WHERE id_przedmiotu = {self.main.class_id} ORDER BY id_sprawdzianu")
            rows = cur.fetchall()
            for row in rows:
                self.comboBox.addItem(row[0])
            cur.execute(f"SELECT CONCAT_WS(' ', imie, nazwisko) FROM uzytkownicy JOIN uzytkownicy_przedmioty ON uzytkownicy.id_uzytkownika = uzytkownicy_przedmioty.id_uzytkownika WHERE id_przedmiotu = {self.main.class_id} ORDER BY uzytkownicy.id_uzytkownika")
            rows = cur.fetchall()
            for row in rows:
    ...
    def getCurrentGrade(self):
        ...
        try:
            ...
            conn = psycopg2.connect(**params)
            cur = conn.cursor()
            cur.execute(f"SELECT ocena FROM oceny JOIN sprawdziany ON oceny.id_sprawdzianu = sprawdziany.id_sprawdzianu JOIN przedmioty ON sprawdziany.id_przedmiotu = przedmioty.id_przedmiotu JOIN uzytkownicy ON oceny.id_ucznia = uzytkownicy.id_uzytkownika WHERE skrot_sprawdzianu = '{self.comboBox.currentText()}' AND CONCAT_WS(' ', imie, nazwisko) = '{self.comboBox_2.currentText()}'")
            row = cur.fetchone()
